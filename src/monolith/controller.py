...
class FaceRecognitionController:
	def __init__(self) -> None:
		...
		self.recognition_enabled: bool = (
			True  # TO SEE ANY ISSUES DEEPFACE MIGHT BE CAUSING
		)
		...
		self.recognition_interval: float = (
			1.5  # YOU MIGHT HAVE TO RAISE THIS IF CPU IS SLOW, VIDEO WILL NOT APPEAR
		)
	...
	def _recognise_current_frame(self) -> None:
		if not self._should_run_recognition():
			return
	...
	def _handle_match(
		...
		liveness_text: str = {True: "passed", False: "failed", None: "not checked"}[
			liveness_result
		]
		emotion_text: str = (
			emotion_result if emotion_result is not None else "not checked"
		)
		self._set_status(
