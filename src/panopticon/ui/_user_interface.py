...
class UserInterface:
	def __init__(
		...
		resolution: str = "".join([str(object=width), "x", str(object=height)])
	...
	def add_feed(self, video_feed: VideoFeed, /, auto_start: bool = False) -> None:
		if getattr(self, "video_widget", False):
	...
	def add_modules(self, models: _Iterable[BaseModule], order: int = 100) -> None:
